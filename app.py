...
# [에러 방지 핵심] pykrx 부품이 없어도 앱이 실행되도록 설정
try:
    ...
    with st.spinner("데이터 분석 중..."):
        ...
        for idx, row in st.session_state.stocks.iterrows():
            ...
            try:
                ...
                analysis_data[tk] = {"df": df, "stop": stop, "code": code, "p_rate": p_rate}
            except: continue
        ...
        if sel in analysis_data:
            ...
            with t2:
                if PYKRX_AVAILABLE and str(target['code']).isdigit():
                    ...
                    st.warning("이 환경에서는 수급 데이터 기능을 사용할 수 없습니다. (Python 버전 호환성)")
else:
